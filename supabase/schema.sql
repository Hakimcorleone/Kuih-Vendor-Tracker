...
create table if not exists vendors (
  ...
  default_commission_type text not null default 'percentage'
    check (default_commission_type in ('percentage', 'fixed_per_item')),
...
create table if not exists batch_items (
  ...
  commission_type text not null default 'percentage'
    check (commission_type in ('percentage', 'fixed_per_item')),
  commission_value numeric not null default 0,
  gross_sales numeric not null default 0,
  shop_commission numeric not null default 0,
...
select
  ...
  coalesce(sum(bi.shop_commission), 0) as shop_commission_today,
...
insert into vendors (name, phone, default_commission_type, default_commission_value, default_leftover_action, notes)
values ('Kak Ani', '0123456789', 'percentage', 20, 'donated_unpaid', 'Sample vendor')
on conflict do nothing;

insert into products (vendor_id, name, default_price, unit, category)
select id, 'Karipap', 2.00, 'pek', 'Kuih'
from vendors where name = 'Kak Ani'
and not exists (select 1 from products where name = 'Karipap' and vendor_id = vendors.id)
limit 1;
